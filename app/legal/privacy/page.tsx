...
export default function PrivacyPage() {
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 20px 96px", lineHeight: 1.8, color: "#1A1A1A" }}>
      ...
      <h2 style={h2style}>11. 共通AI相談員について</h2>
      <p style={pstyle}>
        当サイトの共通AI相談員では、利用者が入力したメッセージ、直近の会話履歴、閲覧中のサービスを示すサービスIDおよび匿名のセッションIDを、当社が運営するchat.todokede.jpに送信します。利用者が連絡を希望し、任意に入力した場合に限り、メールアドレスと直近の会話履歴も送信します。
      </p>
      <p style={pstyle}>
        chat.todokede.jpは、回答生成のため入力メッセージと会話履歴をAnthropic APIへ送信し、利用状況と相談希望の記録にはSupabase、通知にはResendを利用します。これらは回答生成、サービス別の利用状況の把握、お問い合わせ対応および希望者への連絡に利用します。当社はAnthropicの学習利用にオプトインしておらず、APIの入力および出力は標準ではモデルの学習に利用されませんが、不正利用の監視等のため最長30日間保持される場合があります。
      </p>

      <h2 style={h2style}>12. プライバシーポリシーの変更</h2>
      <p style={pstyle}>当社は、法令の変更または事業上の必要に応じて、本ポリシーを変更することがあります。変更後のプライバシーポリシーは、本サイト上に掲載した時点から効力を生じるものとします。</p>

      <h2 style={h2style}>13. お問い合わせ窓口</h2>
      <p style={pstyle}>本ポリシーに関するお問い合わせ、個人情報の取扱いに関するご請求等は、以下の窓口までご連絡ください。</p>
      <p style={pstyle}>MeHer株式会社<br />メールアドレス：plan@todokede.jp</p>

      <p style={{ fontSize: 13, color: "#666666", marginTop: 48 }}>制定日：2026年4月14日 / 最終更新日：2026年7月21日</p>
    </div>
  );
}
