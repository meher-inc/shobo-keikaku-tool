...
const inputStyle: React.CSSProperties = { width: "100%", padding: "12px 16px", fontSize: 16, border: "1px solid var(--border-strong)", borderRadius: 12, outline: "none", background: "var(--surface-input)", boxSizing: "border-box" };
...
export default function ContactPage() {
  ...
  return (
    <main style={{ maxWidth: 640, margin: "0 auto", padding: "clamp(40px,8vw,72px) 16px 64px" }}>
      ...
      {status === "done" ? (
        <div style={{ padding: "24px 24px", background: "var(--ok-bg)", border: "1px solid var(--ok-border)", borderRadius: 16 }}>
          <p style={{ fontSize: 16, fontWeight: 700, color: "var(--ok-text-strong)", margin: "0 0 8px" }}>送信しました</p>
          <p style={{ fontSize: 14, color: "var(--ok-text)", lineHeight: 1.8, margin: 0 }}>
            ご相談ありがとうございます。担当より折り返しご連絡いたします。お急ぎの場合は plan@todokede.jp までご連絡ください。
          </p>
        </div>
      ) : (
        <form onSubmit={onSubmit} style={{ background: "var(--surface)", borderRadius: 20, padding: "28px 24px", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
          {/* ハニーポット（人間には非表示） */}
          <input
            type="text"
            name="website"
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={labelStyle}>会社・団体名<span style={{ color: "var(--err-solid)" }}> *</span></label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ marginBottom: 20 }}>
              <label style={labelStyle}>お名前<span style={{ color: "var(--err-solid)" }}> *</span></label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ marginBottom: 20 }}>
              <label style={labelStyle}>メールアドレス<span style={{ color: "var(--err-solid)" }}> *</span></label>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={labelStyle}>ご相談内容<span style={{ color: "var(--err-solid)" }}> *</span></label>
            <textarea
          ...
          {status === "error" && (
            <p style={{ fontSize: 13, color: "var(--err-solid)", margin: "0 0 16px" }}>{errMsg}</p>
          )}
