...
@~/00_Important_Context/rules/todokede-common.md

# 消防計画 SaaS（shobo-keikaku-tool / plan.todokede.jp）

このリポジトリは **plan.todokede.jp（消防計画作成ツール）** の本体です。Stripe サブスク課金が本番稼働中のため、変更は特に慎重に。共通ルール（recon-first・禁止語・姉妹サービス導線・ファイル境界）は冒頭で import している `todokede-common.md` が正です。npm 使用、launch.json ポート 3400。
