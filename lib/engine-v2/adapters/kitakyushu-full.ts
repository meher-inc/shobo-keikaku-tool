...
import {
  buildKitakyushuAppendices,
  buildKitakyushuAppendixList,
} from "../builders/kitakyushu/appendices";
import { toRenderData } from "./to-render-data";
import kitakyushuCityFull from "../templates/kitakyushu-city.full.json";

/**
 * Build the Kitakyushu City Fire Bureau オフィスビル用（防火・防災）消防計画
 * (第１章〜第４章 / 第１条〜第77条 + 付則) from kitakyushu-city.full.json.
 *
 * 出典: 北九州市「消防計画（中規模、大規模防火対象物用）」
 *   https://www.city.kitakyushu.lg.jp/a92906/business/todokede/yousiki/boukabousaikanri.html
 *
 * 川崎(kawasaki-full)同様の構成。神戸は章・節・条の階層をもつ防火・防災計画。
 * 別表（別表１〜８）は plan !== "light" のとき結合する。
 */

export const KitakyushuFormSchema = z
  .object({
  ...
  .passthrough();
export type KitakyushuForm = z.infer<typeof KitakyushuFormSchema>;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function buildKitakyushuFull(form: any): Promise<Buffer> {
  const loaded = loadPack(kitakyushuCityFull);
  const data = toRenderData((form ?? {}) as Record<string, unknown>);

  extendForKitakyushu(data, form);
  ...
  const allChildren: (Paragraph | Table)[] = [
    ...coverChildren,
    ...packChildren,
    ...(includeAppendix ? buildKitakyushuAppendixList() : []),
    ...(includeAppendix ? buildKitakyushuAppendices(data) : []),
  ];
...
// ── kitakyushu-specific RenderData extension ─────────────────────────

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extendForKitakyushu(data: RenderData, form: any): void {
