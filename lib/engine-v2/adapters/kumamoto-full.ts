...
import {
  buildKumamotoAppendices,
  buildKumamotoAppendixList,
} from "../builders/kumamoto/appendices";
import { toRenderData } from "./to-render-data";
import kumamotoCityFull from "../templates/kumamoto-city.full.json";

/**
 * Build the Kumamoto City Fire Bureau 中規模用 消防計画 (第１〜第10) from
 * kumamoto-city.full.json.
 *
 * 出典: 熊本市「消防計画ひな型（中規模用）」
 *   https://www.city.kumamoto.lg.jp/001/011/014/011/004/p058184.html
 *
 * 札幌(sapporo-full)同様の節建て構成。別表（別表１〜９）は
 * plan !== "light" のとき結合する。
 */

export const KumamotoFormSchema = z
  .object({
  ...
  .passthrough();
export type KumamotoForm = z.infer<typeof KumamotoFormSchema>;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function buildKumamotoFull(form: any): Promise<Buffer> {
  const loaded = loadPack(kumamotoCityFull);
  const data = toRenderData((form ?? {}) as Record<string, unknown>);

  extendForKumamoto(data, form);
  ...
  const allChildren: (Paragraph | Table)[] = [
    ...coverChildren,
    ...packChildren,
    ...(includeAppendix ? buildKumamotoAppendixList() : []),
    ...(includeAppendix ? buildKumamotoAppendices(data) : []),
  ];
...
// ── kumamoto-specific RenderData extension ──────────────────────

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extendForKumamoto(data: RenderData, form: any): void {
