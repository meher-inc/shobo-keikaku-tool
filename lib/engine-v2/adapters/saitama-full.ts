...
import {
  buildSaitamaAppendices,
  buildSaitamaAppendixList,
} from "../builders/saitama/appendices";
import { toRenderData } from "./to-render-data";
import saitamaCityFull from "../templates/saitama-city.full.json";

/**
 * Build the Saitama City Fire Bureau 中規模用 消防計画 (第１〜第10) from
 * saitama-city.full.json.
 *
 * 出典: さいたま市「中規模用消防計画作成例（chukibo-genpon）」
 *   https://www.city.saitama.lg.jp/001/011/014/011/004/p058184.html
 *
 * 札幌(sapporo-full)同様の節建て構成。別表（別表１〜９）は
 * plan !== "light" のとき結合する。
 */

export const SaitamaFormSchema = z
  .object({
  ...
  .passthrough();
export type SaitamaForm = z.infer<typeof SaitamaFormSchema>;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function buildSaitamaFull(form: any): Promise<Buffer> {
  const loaded = loadPack(saitamaCityFull);
  const data = toRenderData((form ?? {}) as Record<string, unknown>);

  extendForSaitama(data, form);
  ...
  const allChildren: (Paragraph | Table)[] = [
    ...coverChildren,
    ...packChildren,
    ...(includeAppendix ? buildSaitamaAppendixList() : []),
    ...(includeAppendix ? buildSaitamaAppendices(data) : []),
  ];
...
// ── saitama-specific RenderData extension ──────────────────────

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extendForSaitama(data: RenderData, form: any): void {
