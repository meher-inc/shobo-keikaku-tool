...
import {
  buildNiigataAppendices,
  buildNiigataAppendixList,
} from "../builders/niigata/appendices";
import { toRenderData } from "./to-render-data";
import niigataCityFull from "../templates/niigata-city.full.json";

/**
 * Build the Niigata City Fire Bureau 防火・単一権原用（甲種）消防計画
 * (第1条〜第11条) from niigata-city.full.json.
 *
 * 出典: 新潟市「消防計画作成例（その他の用途・中規模）」
 *   https://www.city.niigata.jp/bousai/category/291-2-8-7-0-0-0-0-0-0.html
 *
...
export const NiigataFormSchema = z
  .object({
  ...
  .passthrough();
export type NiigataForm = z.infer<typeof NiigataFormSchema>;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function buildNiigataFull(form: any): Promise<Buffer> {
  const loaded = loadPack(niigataCityFull);
  const data = toRenderData((form ?? {}) as Record<string, unknown>);

  extendForNiigata(data, form);
  ...
  const allChildren: (Paragraph | Table)[] = [
    ...coverChildren,
    ...packChildren,
    ...(includeAppendix ? buildNiigataAppendixList() : []),
    ...(includeAppendix ? buildNiigataAppendices(data) : []),
  ];
...
// ── niigata-specific RenderData extension ─────────────────────

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extendForNiigata(data: RenderData, form: any): void {
