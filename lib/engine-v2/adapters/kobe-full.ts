...
import {
  buildKobeAppendices,
  buildKobeAppendixList,
} from "../builders/kobe/appendices";
import { toRenderData } from "./to-render-data";
import kobeCityFull from "../templates/kobe-city.full.json";

/**
 * Build the Kobe City Fire Bureau オフィスビル用（防火・防災）消防計画
 * (第１章〜第４章 / 第１条〜第77条 + 付則) from kobe-city.full.json.
 *
...
export const KobeFormSchema = z
  ...
  .passthrough();
export type KobeForm = z.infer<typeof KobeFormSchema>;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function buildKobeFull(form: any): Promise<Buffer> {
  const loaded = loadPack(kobeCityFull);
  const data = toRenderData((form ?? {}) as Record<string, unknown>);

  extendForKobe(data, form);
  ...
  const allChildren: (Paragraph | Table)[] = [
    ...coverChildren,
    ...packChildren,
    ...(includeAppendix ? buildKobeAppendixList() : []),
    ...(includeAppendix ? buildKobeAppendices(data) : []),
  ];
...
// ── kobe-specific RenderData extension ─────────────────────────

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extendForKobe(data: RenderData, form: any): void {
