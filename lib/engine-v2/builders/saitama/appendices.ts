...
function buildAppendix7FireOrg(data: RenderData): (Paragraph | Table)[] {
  ...
  return [
    ...
    styledTable(["班", "編成（氏名）", "任務"], rows, [2200, 3826, 3000], saitamaTheme),
  ];
}
...
export function buildSaitamaAppendixList(): (Paragraph | Table)[] {
  return [
    pageBreak(),
...
export function buildSaitamaAppendices(data: RenderData): (Paragraph | Table)[] {
  return [
