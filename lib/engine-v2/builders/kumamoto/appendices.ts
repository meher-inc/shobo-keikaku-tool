...
const kumamotoTheme: TableTheme = {
  headerFill: "2E5F9E",
  altFill: "EEF4FA",
};
...
function buildAppendix7FireOrg(data: RenderData): (Paragraph | Table)[] {
  ...
  return [
    ...
    styledTable(["班", "編成（氏名）", "任務"], rows, [2200, 3826, 3000], kumamotoTheme),
  ];
}
...
function buildAppendix9Outsource(): (Paragraph | Table)[] {
  return [
    pageBreak(),
    appendixHeading("９", "防火管理業務の一部委託状況表"),
    styledTable(
      ["項目", "内容"],
      [
        ["受託者（氏名・名称）", B],
...
export function buildKumamotoAppendixList(): (Paragraph | Table)[] {
  return [
    pageBreak(),
...
export function buildKumamotoAppendices(data: RenderData): (Paragraph | Table)[] {
  return [
