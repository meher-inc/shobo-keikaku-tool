...
import { buildKumamotoFull } from "../adapters/kumamoto-full";

/**
 * 熊本市消防局（中規模用）v2 スモークテスト。
 * 出典: 熊本市「消防計画ひな型（中規模用）」
 */
...
async function renderAndUnzip(formOverrides: Record<string, unknown> = {}) {
  const form = {
    building_name: "熊本テストビル",
    prefecture: "熊本県",
    city: "熊本市",
    creation_date_iso: "2026-06-16",
    ...formOverrides,
  };
  const buf = await buildKumamotoFull(form);
...
describe("kumamoto 中規模 v2 smoke tests", () => {
  it("contains 第１〜第９ section headings in order", async () => {
  ...
  it("plan=standard emits 別表1-9; plan=light suppresses them", async () => {
  ...
  it("自衛消防 member names land in 別表7 when supplied", async () => {
    ...
    expect(xml).toContain("熊本太郎");
    expect(xml).toContain("白川花子");
  });
});
