...
function tkA1(d) {
  const out = [pb(), appendixHeading("１", "防火・防災管理業務の一部委託状況表")];
  out.push(tbl(
    ["受託者", "委託業務の範囲", "受託の方法"],
    [
...
function tkA2(d) {
  const out = [pb(), appendixHeading("２", "委託契約書等の内容チェック表")];
  out.push(tbl(
    ["確認事項", "確認結果"],
    [
      ["受託者の資格・能力は適正か", "良 ・ 否"],
...
function tkA3(d) {
  const out = [pb(), appendixHeading("３", "日常の火災予防の担当者と日常の注意事項")];
  out.push(tbl(
    ["担当区域", "担当者", "日常の注意事項"],
    [
...
function tkA4_1(d) {
  const out = [pb(), appendixHeading("４-１", "自主検査チェック表（火気関係）")];
  out.push(tbl(
    ["検査項目", "検査内容", "結果"],
    [
      ["喫煙場所", "吸い殻の処理は適正か", "良 ・ 否"],
  ...
  out.push(sp(), txt(`実施者：${d.daily_checker || "防火管理者"}　　実施時期：毎日`));
  return out;
}

function tkA4_2(d) {
  const out = [pb(), appendixHeading("４-２", "自主検査チェック表（閉鎖障害等）")];
  out.push(tbl(
    ["検査項目", "検査内容", "結果"],
    [
...
function tkA5(d) {
  ...
  out.push(sp(), txt(`実施者：火元責任者　　実施時期：${d.periodic_check_months || "5月と11月"}`));
  return out;
}

function tkA6(d) {
  const out = [pb(), appendixHeading("６", "自主点検チェック表（消防用設備等）")];
  const equipment = (d.fire_equipment && d.fire_equipment.length)
    ? d.fire_equipment
    : ["消火器", "自動火災報知設備", "誘導灯"];
  const rows = equipment.map(eq => [eq, "外観・配置・機能に異常はないか", "良 ・ 否"]);
  out.push(tbl(["設備名", "点検内容", "結果"], rows, [2800, 4200, 2026]));
  out.push(sp(), txt(`実施者：${d.daily_checker || "防火管理者"}`));
  return out;
}

function tkA7(d) {
  const out = [pb(), appendixHeading("７", "自衛消防隊の編成と任務")];
  out.push(tbl(
    ["役割", "氏名", "任務"],
    [
      ["自衛消防隊長", d.manager_name || "(    )", "全体の指揮、消防隊への情報提供"],
      ["副隊長", "(    )", "隊長の補佐、隊長不在時の代行"],
      ["通報連絡班", "(    )", "119番通報、館内・関係者への連絡"],
      ["初期消火班", "(    )", "消火器・屋内消火栓による初期消火"],
      ["避難誘導班", "(    )", "避難経路の確保、在館者の誘導"],
      ["安全防護班", "(    )", "防火戸・防火シャッターの閉鎖確認"],
      ["応急救護班", "(    )", "負傷者の応急手当、救急隊への引継ぎ"],
    ], [2500, 3000, 3526]
  ));
  return out;
}
...
function tkA8(d) {
  const out = [pb(), appendixHeading("８", "転倒・落下・移動防止対策チェック表")];
  out.push(tbl(
    ["対象物", "対策内容", "実施状況"],
    [
...
function tkA9(d) {
  const out = [pb(), appendixHeading("９", "帰宅困難者対策の備蓄一覧")];
  out.push(txt("　従業員等の3日分（東京都帰宅困難者対策条例に基づく）", { size: 18, color: "666666" }));
  out.push(tbl(
    ["品目", "備蓄量の目安", "現在の備蓄量"],
    [
      ["飲料水", "1人1日3L × 3日 = 9L", "(    ) L"],
...
function tkA10(d) {
  const out = [pb(), appendixHeading("10", "時差退社計画")];
  out.push(tbl(
    ["時間帯", "退社対象者", "備考"],
    [
...
function tkA11(d) {
  const out = [pb(), appendixHeading("11", "施設安全点検チェックリスト")];
  out.push(txt("　地震発生後、施設の使用可否を判断するための点検", { size: 18, color: "666666" }));
  out.push(tbl(
    ["点検箇所", "点検内容", "結果"],
    [
...
function buildAllTokyoAppendices(d) {
  const all = [];
...
function generateTokyoPlan(d) {
  ...
  if (d.include_appendix) {
    c.push(...buildAllTokyoAppendices(d));
  }
...
if (require.main === module) {
  buildTokyo(sample).then(buf => {
    fs.writeFileSync("./tokyo_shobo_keikaku_full.docx", buf);
    console.log("OK: tokyo_shobo_keikaku_full.docx");
  });
}

module.exports = { generateTokyoPlan, buildTokyo, build: buildTokyo };
