...
1. `.env.local` に `STRIPE_COUPON_LINE_WAITLIST=LINE_WAITLIST_FIRST_MONTH_FREE` が設定されていることを確認
2. Stripe Dashboard で coupon `LINE_WAITLIST_FIRST_MONTH_FREE` を作成（duration: once, percent_off: 100）
3. `/api/subscribe` に coupon 付きで POST:
   ```bash
   ...
   ```
4. 返された URL で Stripe Checkout を開き、テストカード `4242 4242 4242 4242` で決済
5. **確認**: Checkout 画面で初月が ¥0 と表示される
6. **確認**: `customer.subscription.created` が流れ、Supabase にレコード作成
7. **確認**: `/subscribe/success` でプラン詳細・リリース告知が表示される

### シナリオ 7: 無効なクーポンコードのエラーハンドリング

1. 存在しないクーポンコードで POST:
   ```bash
   curl -X POST http://localhost:3000/api/subscribe \
     -H 'Content-Type: application/json' \
     -d '{"planId":"minimum","billingCycle":"monthly","customerEmail":"test@example.com","coupon":"INVALID_CODE"}'
   ```
2. **確認**: 500 エラーが返る（Stripe が無効クーポンを拒否）
